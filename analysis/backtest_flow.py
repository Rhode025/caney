...
the 3-mph rule (miles/3 hours), auto-calibrated to the Stonewall gauge. Stonewall (15 mi) is
...
FIXED deployed kernel and compares to the real USGS Stonewall gauge, then:
  ...
  C. per-event response  (leading-edge rise lag vs the 3-mph=5h rule; peak lag; attenuation)
...
# ── the deployed model constants (verbatim from briefing.py) ──
...
LEAD_3MPH = MFD_STONE / WATER_MPH                        # 3-mph leading edge to Stonewall = 5.0 h
...
print("\nMODEL CONSTANTS  baseflow=%.0f  kernel centroid=%.2f h  3-mph leading edge=%.1f h" % (BASEFLOW, CENTROID, LEAD_3MPH))
...
for label, pred in [("nominal baseflow 375", M), ("baseflow %+.0f (auto-cal)" % CALIB_ADJ, Mc)]:
    s = stats(G, pred)
...
if evs:
    ...
    print("  %d events  |  rise lag: median %d h (range %d–%d)   [3-mph rule says %.0f h]"
          % (len(evs), med(rl), rl[0], rl[-1], LEAD_3MPH))
...
try:
    ...
    print("  fit on %d hrs   implied baseflow %.0f (model 375)   steady-state gain Σh = %.2f (model 1.00)" % (en, eb, gain))
