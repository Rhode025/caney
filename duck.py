...
for i in range(6):
    ...
    outlook.append({"label":("Today" if i==0 else d.strftime("%a")),"date":d.strftime("%-m/%-d"),
...
COORD={"Riverside":(35.63641,-87.03379),"Chickasaw Trace":(35.66285,-87.09328),"Williamsport":(35.68417,-87.25116),
...
_dord=[("Riverside",0.025),("Chickasaw",0.131),("Williamsport",0.345),("Leatherwood",0.653),("Littlelot",0.742),("Centerville",1.0)]
TIMELINE=({"times":[p["t"] for p in _ft],"nowFrame":max(0,_now),"unit":"kcfs","dec":2,"refIdx":5,"refName":"Centerville",
  "front":False,"frontThresh":0,"srcLabel":"Columbia ↑","mouthLabel":"Centerville ↓",
...
DATA={"today":now_ct.strftime("%A, %B %-d · %-I:%M %p"),"flysel":FLYSEL,"colflow":col_flow,"solunar":SOL,"hatch":HATCH,"month":now_ct.month,"chatter":riverlib.load_intel("duck"),"timeline":TIMELINE,
      "sections":sections,"suggest":FLOAT,"craft":CRAFT,"craft0":CRAFT0,
...
@media(max-width:680px){.app{padding:22px 14px 60px}h1{font-size:28px}.wx .m{min-width:0}}
 ...
 __SWITCHER__
 <div class="eyebrow">Smallmouth planner · middle Duck, Columbia→Centerville</div>
 <h1>Duck River</h1><div class="cap" id="cap"></div>
...
__FLOWTIMER_JS__
__FLYMATRIX_JS__
document.getElementById('cap').innerHTML=D.today+' · Duck River at Centerville gauge (NWS CNVT1)';
